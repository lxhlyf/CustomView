...
public class MusicManager {
    ...
    public void playMusic(MusicInfo musicInfo) {
        ...
        List<MusicInfo> list = new ArrayList<>(1);
        list.add(musicInfo);
     ...
     */
    public void playMusicList(final List<MusicInfo> list, final int playIndex) {
        // 数据为空
        ...
        // 当前没有播放数据
            ...
            // 播放队列数据,发到Service，交到Service的队列中
     ...
     * @param playIndex 当前播放位置
     */
    public void updatePlayList(final List<MusicInfo> list, final int playIndex) {
    ...
    public void init(Context context) {
        ...
        this.mMediaBrowser = new MediaBrowserCompat(
                ...
                // 音频连接状态回调， 当在Service的setSessionToken的时候会回调这个方法。
                mConnectionCallback,
                //
                null);
        // 链接service
     ...
     * 根据uid 判断给定的包名
     */
    private boolean isValidPackage(String pkg, int uid) { //是否是正当的有效的包名
        // 包名为空
        ...
        final String[] packages = pm.getPackagesForUid(uid); //取得这个Uid下的所有的包
        final int N = packages.length;
        for (int i = 0; i < N; i++) {
            if (packages[i].equals(pkg)) {
                return true; //如果包中有，就返回为true
            }
        }
        return false;
    }
